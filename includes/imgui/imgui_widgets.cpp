...
const char* ImGui::GetKeyNameExtended(int key)
{
    ...
    default:
        // For all other keys, use ImGui::GetKeyName
        return ImGui::GetKeyName((ImGuiKey)key);
    }
}

void ImGui::Hotkey(int* keyV)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
	...
	ImGuiContext& g = *GImGui;
    const ImVec2 hotkeyTextSize = CalcTextSize(GetKeyNameExtended(ImGuiKey(*keyV)));
    static bool waitingforkey = false;
    int key = *keyV;
    if (!waitingforkey) {
        if (ImGui::Button(GetKeyNameExtended(ImGuiKey(*keyV)), ImVec2(hotkeyTextSize.x + 10, hotkeyTextSize.y + 10)))
            waitingforkey = true;
    }
    else if (waitingforkey) {
        ImGui::Button("...", ImVec2(hotkeyTextSize.x + 10, hotkeyTextSize.y + 10));
        for (auto i = 0; i < 5; i++) {
            if (g.IO.MouseDown[i]) {
                switch (i) {
                case 0:
