...
namespace ImGui
{
    ...
    IMGUI_API void          Hotkey(int* key);
    IMGUI_API const char*   GetKeyNameExtended(int key);
    ...
    IMGUI_API ImVec2        CalcTextSizeFont(float font_size, const char* text, const char* text_end = NULL, bool hide_text_after_double_hash = false, float wrap_width = -1.0f);
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddOutlinedRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, ImU32 outcol, float rounding = 0.0f, ImDrawFlags flags = 0, float thickness = 1.0f);
    ...
    IMGUI_API void  AddOutlinedText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, ImU32 outcol, const char* text_begin, const char* text_end = NULL);
